...
const TEAM = [
  { name: "Tahir Sajeel Farooq", role: "Co-Founder", initials: "TSF", color: C.siteGreen, img: "/founder2.png" },
  { name: "Maheen Shafiq", role: "Co-Founder", initials: "MS", color: C.siteBrown, img: "/founder1.png" },
  { name: "Hamayoon Shah", role: "Chief Research Officer", initials: "CRO", color: C.siteGreenMuted, img: "/research.png" },
/*{ name: "Research Assistant", role: "Research Assistant", initials: "RA", color: C.siteOlive },
  { name: "Design Associate", role: "Design Associate", initials: "DA", color: C.siteSage }, 
  { name: "Strategy Associate", role: "Strategy Associate", initials: "SA", color: C.siteBrownLight, img: null }, */
];
//-- Events --
// content lives in src/shared/eventsData.js — the single source of truth
// this site and the app both read (SPEC.md §Events + Calendar)
import { EVENTS } from "./shared/eventsData.js";
...
function BlogArticlePage({ blog, onBack }) {
  ...
  const tagColors = { Research: C.siteBrown, Stories: C.siteGreen, "Well-being": C.siteOlive };
  const tc = tagColors[blog.tag] || C.siteGreen;

  return (
    <div style={{ fontFamily: FONTS.sans, color: C.siteTextMain, background: C.siteWhite, minHeight: "100vh" }}>
      <style>{`
      ...
      {/* Back bar */}
      <div style={{ background: C.siteGreen, padding: "14px 24px", position: "sticky", top: 0, zIndex: 100, height: 64, display: "flex", alignItems: "center" }} >
        <div style={{ maxWidth: 760, margin: "0 auto", display: "flex", alignItems: "center", gap: 12, width: "100%" }} >
          <button
            onClick={() => onBack(null)} style={{ background: "rgba(250,243,233,0.12)", border: "none", borderRadius: 40, padding: "8px 20px", color: C.siteCream, fontFamily: FONTS.sans, fontWeight: 600, fontSize: 14, cursor: "pointer", transition: "background 0.3s" }}
          ...
          <button
            onClick={handleShare} style={{ background: copied ? "rgba(250,243,233,0.28)" : "rgba(250,243,233,0.12)", border: "none", borderRadius: 40, padding: "8px 18px", color: C.siteCream, fontFamily: FONTS.sans, fontWeight: 600, fontSize: 13, cursor: "pointer", transition: "background 0.3s", display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}
          ...
          >
            {copied ? (
              ...
              <>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={C.siteCream} strokeWidth="2"><path d="M18 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6zM6 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6zM18 22a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"/><path d="M8.6 13.5l6.8 3.9M15.4 6.6L8.6 10.5"/></svg>
                Share
              </>
            )}
          </button>
      ...
      <div style={{ position: "relative" }}>
        {blog.coverImg ? (
          ...
          <div
            ...
            <div
              ...
              <div style={{ maxWidth: 760, margin: "0 auto" }}>
                <span
                  style={{ fontSize: 11, fontWeight: 700, color: C.siteWhite, background: tc, padding: "5px 14px", borderRadius: 20, textTransform: "uppercase", letterSpacing: "0.08em", display: "inline-block", marginBottom: 16 }} >
                  {blog.tag}
                </span>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div
                    ...
                    <span style={{ fontSize: 14, fontWeight: 700, color: C.siteWhite }} > H </span>
                  </div>
                  <div>
          ...
          // ── Without cover image ──
          <div
            style={{ background: C.siteBg, padding: "64px 24px 48px", borderBottom: `1px solid ${C.siteWarmGray}40` }} >
            <div style={{ maxWidth: 760, margin: "0 auto" }}>
              <span
              ...
              <h1 style={{ fontFamily: FONTS.serif, fontSize: "clamp(1.8rem,4vw,2.8rem)", color: C.siteGreen, fontWeight: 700, lineHeight: 1.2, marginBottom: 20 }} > {blog.title} </h1>
              <p style={{ fontSize: 18, color: C.siteTextMuted, lineHeight: 1.6, marginBottom: 28, fontStyle: "italic", fontFamily: FONTS.serif }} > {blog.excerpt} </p>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div
                ...
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: C.siteTextMain }} > {blog.author} </div>
                  <div style={{ fontSize: 12, color: C.siteTextMuted }} > {blog.date} · {blog.readTime} </div>
                </div>
              </div>
            </div>
          </div>
        ...
        {blog.coverImg && (
          <div style={{ background: C.siteBg, padding: "36px 24px 32px", borderBottom: `1px solid ${C.siteWarmGray}40` }}  >
            <div style={{ maxWidth: 760, margin: "0 auto" }}>
              <h1 style={{ fontFamily: FONTS.serif, fontSize: "clamp(1.8rem,4vw,2.8rem)", color: C.siteGreen, fontWeight: 700, lineHeight: 1.2, marginBottom: 12 }} > {blog.title} </h1>
              <p style={{ fontSize: 18, color: C.siteTextMuted, lineHeight: 1.6, fontStyle: "italic", fontFamily: FONTS.serif }} > {blog.excerpt} </p>
            </div>
          </div>
        )}
      </div>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "52px 24px 0" }} className="article-body">
        {blog.content.map((block, i) => {
          if (block.type === "lead") return (
            <p key={i} style={{ fontSize: "clamp(1.1rem,2vw,1.25rem)", lineHeight: 1.8, color: C.siteTextMain, fontWeight: 500, marginBottom: "1.6em", fontFamily: FONTS.serif }}>{block.text}</p>
          );
          if (block.type === "paragraph") return (
            <p key={i} style={{ fontSize: 17, lineHeight: 1.85, color: C.siteTextMuted, marginBottom: "1.6em" }}>{block.text}</p>
          );
            ...
            <div key={i} style={{ margin: "2em 0" }}>
              {block.items.map((item, j) => (
                <div key={j} style={{ display: "flex", gap: 20, marginBottom: 28, alignItems: "flex-start" }}>
                  <div style={{ width: 36, height: 36, borderRadius: "50%", background: tc, color: C.siteCream, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 15, flexShrink: 0, marginTop: 2 }}>{j + 1}</div>
                  <div>
                    <p style={{ fontSize: 17, fontWeight: 700, color: C.siteGreen, marginBottom: 6 }}>{item.title}</p>
                    <p style={{ fontSize: 16, lineHeight: 1.8, color: C.siteTextMuted, margin: 0 }}>{item.text}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* Watermark footer */}
      <div style={{ maxWidth: 760, margin: "48px auto 0", padding: "24px 24px 48px", borderTop: `1px solid ${C.siteWarmGray}50`, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <img src="/logo-small.png" alt="Saathban" style={{ height: 32, width: "auto", opacity: 0.7 }} />
          <div>
            <div style={{ fontSize: 12, fontWeight: 600, color: C.siteTextMuted }}>Saathban — Timeless Togetherness</div>
            <div style={{ fontSize: 11, color: C.siteWarmGray }}>saathban.com</div>
          </div>
        </div>
        <span style={{ fontSize: 12, color: C.siteWarmGray, fontStyle: "italic" }}>Where generations flourish together</span>
      </div>

      {/* More articles */}
      <div style={{ background: C.siteBg, padding: "48px 24px" }}>
        <div style={{ maxWidth: 760, margin: "0 auto" }}>
          <h3 style={{ fontFamily: FONTS.serif, fontSize: 20, color: C.siteGreen, marginBottom: 24 }}>More from Saathban</h3>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {BLOGS.filter(b => b.id !== blog.id).slice(0, 3).map(b => (
              <div key={b.id} onClick={() => { window.scrollTo(0, 0); onBack(b); }}
                style={{ display: "flex", gap: 16, alignItems: "flex-start", cursor: "pointer", padding: "16px", borderRadius: 12, background: C.siteWhite, boxShadow: "0 1px 8px rgba(6,50,20,0.05)", transition: "all 0.2s" }}
                ...
                <div style={{ width: 48, height: 48, borderRadius: 10, background: `${tagColors[b.tag] || C.siteGreen}12`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <span style={{ fontFamily: FONTS.serif, fontSize: 20, fontWeight: 700, color: tagColors[b.tag] || C.siteGreen }}>{b.tag[0]}</span>
                </div>
                <div>
                  <span style={{ fontSize: 11, fontWeight: 700, color: tagColors[b.tag] || C.siteGreen, textTransform: "uppercase", letterSpacing: "0.06em" }}>{b.tag} · {b.readTime}</span>
                  <p style={{ fontSize: 15, fontWeight: 600, color: C.siteGreen, margin: "4px 0 2px", lineHeight: 1.35 }}>{b.title}</p>
                  <p style={{ fontSize: 13, color: C.siteTextMuted, margin: 0 }}>{b.date}</p>
                </div>
              </div>
            ))}
          </div>
          <div style={{ textAlign: "center", marginTop: 28 }}>
            <button onClick={() => onBack(null)} style={{ background: "transparent", border: `2px solid ${C.siteGreen}`, borderRadius: 40, padding: "10px 28px", color: C.siteGreen, fontFamily: FONTS.sans, fontWeight: 600, fontSize: 14, cursor: "pointer" }}>← All Articles</button>
          </div>
        </div>
      </div>

      <div style={{ background: C.siteGreen, padding: "24px", textAlign: "center" }}>
    ...
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 16, marginTop: 32 }}>
        <button onClick={prev} disabled={idx === 0}
          style={{ width: 40, height: 40, borderRadius: "50%", border: `2px solid ${idx === 0 ? C.siteWarmGray : C.siteGreen}`, background: "transparent", cursor: idx === 0 ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.2s", color: idx === 0 ? C.siteWarmGray : C.siteGreen, fontSize: 18, fontWeight: 700 }}>
          ←
        </button>
        <div style={{ display: "flex", gap: 8 }}>
          {Array.from({ length: total }).map((_, i) => (
            <div key={i} onClick={() => setIdx(i)}
              style={{ width: i === idx ? 24 : 8, height: 8, borderRadius: 4, background: i === idx ? C.siteGreen : C.siteWarmGray, cursor: "pointer", transition: "all 0.3s ease" }} />
          ))}
        </div>
        <button onClick={next} disabled={idx === total - 1}
          style={{ width: 40, height: 40, borderRadius: "50%", border: `2px solid ${idx === total - 1 ? C.siteWarmGray : C.siteGreen}`, background: "transparent", cursor: idx === total - 1 ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.2s", color: idx === total - 1 ? C.siteWarmGray : C.siteGreen, fontSize: 18, fontWeight: 700 }}>
          →
        </button>
      </div>
    </div>
...
function SecTitle({ children, sub, light }) {
  return (
    <div style={{ marginBottom: 52, textAlign: "center" }}>
      <h2 style={{ fontFamily: FONTS.serif, fontSize: "clamp(1.9rem,4vw,2.8rem)", color: light ? C.siteCream : C.siteGreen, margin: 0, fontWeight: 700, letterSpacing: "-0.01em" }}>{children}</h2>
      {sub && <p style={{ fontSize: 16, color: light ? "rgba(250,243,233,0.75)" : C.siteTextMuted, marginTop: 14, maxWidth: 540, marginInline: "auto", lineHeight: 1.65 }}>{sub}</p>}
      <div style={{ display: "flex", gap: 6, justifyContent: "center", marginTop: 20 }}>
        <div style={{ width: 32, height: 3, borderRadius: 2, background: C.siteBrown }} />
        <div style={{ width: 12, height: 3, borderRadius: 2, background: C.siteGreen }} />
        <div style={{ width: 6, height: 3, borderRadius: 2, background: C.siteSage }} />
      </div>
    </div>
  );
}
...
function Btn({ children, onClick, variant = "primary", style: s = {}, href }) {
  ...
  const styles = variant === "primary"
    ? { ...base, background: C.siteGreen, color: C.siteCream, boxShadow: "0 4px 24px rgba(6,50,20,0.25)", ...s }
    : variant === "brown"
      ? { ...base, background: C.siteBrown, color: C.siteCream, boxShadow: "0 4px 24px rgba(87,52,37,0.25)", ...s }
      : { ...base, background: "transparent", color: C.siteGreen, border: `2px solid ${C.siteGreen}`, ...s };
...
function Card({ children, style: s = {}, hover = true, onClick }) {
  return (
    <div onClick={onClick} style={{ background: C.siteWhite, borderRadius: 16, padding: 32, boxShadow: "0 2px 20px rgba(6,50,20,0.06)", transition: "all 0.35s ease", cursor: onClick ? "pointer" : "default", ...s }}
...
function Stat({ number, label, delay = 0 }) {
  return (
    <FadeIn delay={delay} style={{ textAlign: "center", flex: "1 1 140px" }}>
      <div style={{ fontFamily: FONTS.serif, fontSize: "clamp(2rem,4vw,3rem)", fontWeight: 700, color: C.siteBrown }}>{number}</div>
      <div style={{ fontSize: 14, color: C.siteTextMuted, marginTop: 6, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.08em" }}>{label}</div>
    </FadeIn>
  );
}
...
function ImageCarousel() {
  ...
  return (
    <section style={{ padding: "32px 0", background: C.siteBg }}>
      <div style={{ maxWidth: 1160, margin: "0 auto", padding: "0 24px" }}>
        <FadeIn>
          <div style={{ position: "relative", borderRadius: 16, overflow: "hidden", boxShadow: "0 4px 24px rgba(6,50,20,0.10)" }}>
            <div style={{ position: "relative", height: "clamp(220px, 36vw, 480px)", overflow: "hidden", background: C.siteDark }}>
              <img
                key={current}
                src={img.src}
                alt={img.caption}
              ...
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12,}}>
                  <span style={{
                    ...
                    color: C.siteCream, background: C.siteGreen, color: C.siteCream, padding: "5px 12px", fontSize: 11, backdropFilter: "blur(6px)",
                ...
                <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                  {CAROUSEL_IMAGES.map((_, i) => (
                    <div key={i} onClick={() => go(i, i > current ? 1 : -1)} style={{
                      ...
                      background: i === current ? C.siteCream : "rgba(250,243,233,0.3)",
              ...
              {[{ label: "←", action: prev, side: { left: 14 } }, { label: "→", action: next, side: { right: 14 } }].map(({ label, action, side }) => (
                <button key={label} onClick={action} style={{
                  ...
                  color: C.siteCream, fontSize: 16, fontWeight: 700, cursor: "pointer",
...
function HeroIllustration() {
  return (
    <svg viewBox="0 0 400 440" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width: "100%", maxWidth: 380, height: "auto" }}>
      {/* Background circles */}
      <circle cx="200" cy="220" r="190" fill={C.siteGreen} opacity="0.06" />
      <circle cx="200" cy="220" r="140" fill={C.siteBrown} opacity="0.06" />

      {/* Ground */}
      <ellipse cx="200" cy="400" rx="160" ry="16" fill={C.siteGreen} opacity="0.08" />

      {/* Floating leaves */}
      <path d="M80 120 Q90 110, 100 120 Q90 130, 80 120Z" fill={C.siteGreen} opacity="0.2" transform="rotate(-30 90 120)" />
      <path d="M300 100 Q310 90, 320 100 Q310 110, 300 100Z" fill={C.siteSage} opacity="0.2" transform="rotate(20 310 100)" />
      <path d="M60 300 Q70 290, 80 300 Q70 310, 60 300Z" fill={C.siteGreen} opacity="0.15" transform="rotate(-15 70 300)" />

      {/* Small birds */}
      <path d="M100 80 Q105 70 110 78 Q115 70 120 80" stroke={C.siteGreen} strokeWidth="1.5" fill="none" opacity="0.2" />
      <path d="M280 60 Q285 50 290 58 Q295 50 300 60" stroke={C.siteGreen} strokeWidth="1.5" fill="none" opacity="0.15" />
...
function EventDetailPage({ event, onBack }) {
  ...
  return (
    <div style={{ fontFamily: FONTS.sans, color: C.siteTextMain, background: C.siteBg, minHeight: "100vh" }}>
      <style>{`
      ...
      {/* Back bar */}
      <div style={{ background: C.siteGreen, padding: "14px 24px", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", alignItems: "center", gap: 12 }}>
          <button onClick={onBack} style={{ background: "rgba(250,243,233,0.12)", border: "none", borderRadius: 40, padding: "8px 20px", color: C.siteCream, fontFamily: FONTS.sans, fontWeight: 600, fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", gap: 8, transition: "background 0.3s" }}
            onMouseEnter={e => e.currentTarget.style.background = "rgba(250,243,233,0.22)"}
            onMouseLeave={e => e.currentTarget.style.background = "rgba(250,243,233,0.12)"}>
            ← Back to Events
          </button>
      ...
      {/* Hero banner */}
      <div style={{ background: `linear-gradient(135deg, ${C.siteGreen}, ${C.siteGreenLight})`, padding: "64px 24px 56px", textAlign: "center", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", top: -60, right: -60, width: 240, height: 240, borderRadius: "50%", border: "1px solid rgba(250,243,233,0.08)" }} />
        <div style={{ position: "absolute", bottom: -40, left: -40, width: 180, height: 180, borderRadius: "50%", background: `${C.siteBrown}15` }} />
        <div style={{ position: "relative", zIndex: 1, maxWidth: 700, margin: "0 auto" }}>
          <span style={{ display: "inline-block", background: `${C.siteBrown}`, borderRadius: 40, padding: "6px 18px", marginBottom: 20, fontSize: 12, fontWeight: 700, color: C.siteCream, letterSpacing: "0.08em", textTransform: "uppercase" }}>Past Event</span>
          <h1 style={{ fontFamily: FONTS.serif, fontSize: "clamp(1.8rem,4vw,3rem)", color: C.siteCream, fontWeight: 700, lineHeight: 1.2, marginBottom: 16 }}>{event.title}</h1>
      ...
      <div style={{ ...px, padding: "60px 24px" }}>
        ...
        <FadeIn>
          <div style={{ marginBottom: 56 }}>
            <h2 style={{ fontFamily: FONTS.serif, fontSize: 26, color: C.siteGreen, marginBottom: 20 }}>About the Event</h2>
            {d.about.split("\n\n").map((para, i) => (
              <p key={i} style={{ fontSize: 16, lineHeight: 1.85, color: C.siteTextMuted, marginBottom: 16 }}>{para}</p>
            ))}
          </div>
        </FadeIn>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32, marginBottom: 56 }} className="event-detail-grid">
          <FadeIn delay={0.1}>
            <div style={{ background: C.siteWhite, borderRadius: 16, padding: 32, boxShadow: "0 2px 20px rgba(6,50,20,0.06)" }}>
              <h3 style={{ fontFamily: FONTS.serif, fontSize: 20, color: C.siteGreen, marginBottom: 20 }}>Event Highlights</h3>
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {d.highlights.map((h, i) => (
                  <div key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                    <div style={{ width: 22, height: 22, borderRadius: "50%", background: `${C.siteBrown}15`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 1 }}>
                      <div style={{ width: 7, height: 7, borderRadius: "50%", background: C.siteBrown }} />
                    </div>
                    <span style={{ fontSize: 14, color: C.siteTextMuted, lineHeight: 1.6 }}>{h}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <FadeIn delay={0.15}>
            <div style={{ background: C.siteWhite, borderRadius: 16, padding: 32, boxShadow: "0 2px 20px rgba(6,50,20,0.06)" }}>
              <h3 style={{ fontFamily: FONTS.serif, fontSize: 20, color: C.siteGreen, marginBottom: 20 }}>Event Agenda</h3>
              <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
                {d.agenda.map((a, i) => (
                  <div key={i} style={{ display: "flex", gap: 16, alignItems: "flex-start", paddingBottom: i < d.agenda.length - 1 ? 16 : 0, borderBottom: i < d.agenda.length - 1 ? `1px solid ${C.siteWarmGray}50` : "none", marginBottom: i < d.agenda.length - 1 ? 16 : 0 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: C.siteBrown, whiteSpace: "nowrap", paddingTop: 2, minWidth: 54 }}>{a.time}</span>
                    <span style={{ fontSize: 14, color: C.siteTextMuted, lineHeight: 1.55 }}>{a.item}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <FadeIn delay={0.2}>
          <div style={{ marginBottom: 56 }}>
            <h2 style={{ fontFamily: FONTS.serif, fontSize: 26, color: C.siteGreen, marginBottom: 8 }}>Gallery</h2>
            <p style={{ fontSize: 14, color: C.siteTextMuted, marginBottom: 24, fontStyle: "italic" }}>Photos from the event.</p>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 16 }}>
              {d.gallery.map((g, i) => (
                <div key={i} style={{ background: `linear-gradient(135deg, ${C.siteGreen}10, ${C.siteBrown}06)`, borderRadius: 14, aspectRatio: "4/3", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10, border: `1px dashed ${C.siteWarmGray}` }}>
                  <span style={{ fontSize: 36 }}>{g.emoji}</span>
                  <span style={{ fontSize: 13, color: C.siteTextMuted, fontWeight: 500 }}>{g.label}</span>
                  <span style={{ fontSize: 11, color: C.siteWarmGray, fontStyle: "italic" }}>Add photo</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Quote */}
        <FadeIn delay={0.25}>
          <div style={{ background: `linear-gradient(135deg, ${C.siteGreen}08, ${C.siteBrown}06)`, borderRadius: 20, padding: "44px 48px", textAlign: "center", border: `1px solid ${C.siteWarmGray}50`, marginBottom: 56 }}>
            <div style={{ fontSize: 48, color: C.siteBrown, opacity: 0.2, fontFamily: FONTS.quote, lineHeight: 1, marginBottom: 8 }}>"</div>
            <p style={{ fontFamily: FONTS.serif, fontSize: "clamp(1.1rem,2.5vw,1.4rem)", fontStyle: "italic", color: C.siteGreen, lineHeight: 1.7, maxWidth: 620, margin: "0 auto 16px" }}>
              {d.quote.text}
            </p>
            <span style={{ fontSize: 14, color: C.siteTextMuted, fontWeight: 600 }}>— {d.quote.author}</span>
          </div>
        </FadeIn>
        ...
        <FadeIn delay={0.3}>
          <div style={{ textAlign: "center" }}>
            <p style={{ fontSize: 16, color: C.siteTextMuted, marginBottom: 20 }}>Want to be part of our next event?</p>
      ...
      {/* Footer strip */}
      <div style={{ background: C.siteGreen, padding: "24px", textAlign: "center", marginTop: 40 }}>
...
export default function Saathban() {
  ...
  // Preload all blog cover images at high priority
  useEffect(() => {
    BLOGS.forEach(({ coverImg }) => {
      if (!coverImg) return;
      if (document.querySelector(`link[rel="preload"][href="${coverImg}"]`)) return;
      const link = document.createElement("link");
      link.rel = "preload";
      link.as = "image";
      link.fetchPriority = "high";
...
// Blog article page
  ...
  return (
    <div style={{ fontFamily: FONTS.sans, color: C.siteTextMain, background: C.siteBg, minHeight: "100vh", overflowX: "hidden" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        body { background: ${C.siteBg}; }
        ::selection { background: ${C.siteGreen}; color: ${C.siteCream}; }
        input:focus, textarea:focus { outline: 2px solid ${C.siteGreen}; outline-offset: 2px; }
        ...
        a { color: ${C.siteGreen}; text-decoration: none; }
        a:hover { color: ${C.siteGreenLight}; }
      `}</style>
      ...
      <nav style={{
        ...
        borderBottom: scrolled ? `1px solid ${C.siteGreen}15` : "none",
        ...
        <div style={{ ...px, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div className="hide-mobile" style={{ display: "flex", gap: 28, alignItems: "center" }}>
            {NAV_ITEMS.filter(n => n.id !== "involve").map(n => (
              <span key={n.id} onClick={() => scrollTo(n.id)} style={{
                cursor: "pointer", fontSize: 13.5, fontWeight: 600, color: C.siteBrown, letterSpacing: "0.05em",
                textTransform: "uppercase", transition: "color 0.3s", padding: "4px 0",
              }} onMouseEnter={e => e.target.style.color = C.siteGreen} onMouseLeave={e => e.target.style.color = C.siteBrown}>{n.label}</span>
            ))}
          ...
          <div className="show-mobile" style={{ display: "none", cursor: "pointer", flexDirection: "column", gap: 5, padding: 8 }} onClick={() => setMenuOpen(!menuOpen)}>
            <div style={{ width: 24, height: 2.5, background: C.siteGreen, borderRadius: 2, transition: "all 0.3s", transform: menuOpen ? "rotate(45deg) translate(5px,5px)" : "none" }} />
            <div style={{ width: 24, height: 2.5, background: C.siteGreen, borderRadius: 2, transition: "all 0.3s", opacity: menuOpen ? 0 : 1 }} />
            <div style={{ width: 24, height: 2.5, background: C.siteGreen, borderRadius: 2, transition: "all 0.3s", transform: menuOpen ? "rotate(-45deg) translate(5px,-5px)" : "none" }} />
          </div>
        </div>
        ...
        {/* Mobile menu */}
        {menuOpen && (
          <div style={{ background: C.siteCream, borderTop: `1px solid ${C.siteWarmGray}`, padding: "20px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
            {NAV_ITEMS.map(n => (
              <span key={n.id} onClick={() => scrollTo(n.id)} style={{ fontSize: 16, fontWeight: 600, color: C.siteGreen, cursor: "pointer", padding: "8px 0", borderBottom: `1px solid ${C.siteWarmGray}` }}>{n.label}</span>
            ))}
          </div>
        )}
      </nav>
      ...
      <section id="home" style={{ position: "relative", minHeight: "100vh", display: "flex", alignItems: "center", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", inset: 0, zIndex: 0 }}>
          <div style={{ position: "absolute", top: "-10%", right: "-5%", width: "55vw", height: "55vw", maxWidth: 700, maxHeight: 700, borderRadius: "50%", background: `radial-gradient(circle, ${C.siteGreen}08, transparent 70%)` }} />
          <div style={{ position: "absolute", bottom: "5%", left: "-8%", width: "40vw", height: "40vw", maxWidth: 500, maxHeight: 500, borderRadius: "50%", background: `radial-gradient(circle, ${C.siteBrown}08, transparent 70%)` }} />
          <div className="hide-mobile" style={{ position: "absolute", top: "20%", left: "65%", width: 160, height: 160, borderRadius: "50%", border: `2px solid ${C.siteGreen}12`, animation: "float 6s ease-in-out infinite" }} />
          {/* Leaf pattern */}
          <svg style={{ position: "absolute", top: "15%", left: "5%", opacity: 0.05 }} width="120" height="120">
            {[0, 1, 2, 3].map(i => <path key={i} d={`M${30 + i * 25} 60 Q${40 + i * 25} ${40 - i * 5}, ${50 + i * 25} 60 Q${40 + i * 25} ${80 + i * 5}, ${30 + i * 25} 60Z`} fill={C.siteGreen} transform={`rotate(${i * 30} ${40 + i * 25} 60)`} />)}
          </svg>
        </div>
        ...
        <div style={{ ...px, position: "relative", zIndex: 1, width: "100%", paddingTop: 100, paddingBottom: 60 }}>
          <div className="hero-flex" style={{ display: "flex", alignItems: "center", gap: 48 }}>
            ...
            <div style={{ flex: "1 1 480px", maxWidth: 580 }}>
              <div style={{ display: "inline-block", background: `${C.siteGreen}10`, borderRadius: 40, padding: "8px 20px", marginBottom: 24 }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: C.siteGreen, letterSpacing: "0.08em", textTransform: "uppercase" }}>Where Generations Flourish Together</span>
              </div>
              <h1 className="hero-text" style={{ fontFamily: FONTS.serif, fontSize: "clamp(2.6rem,5.5vw,4.2rem)", color: C.siteGreen, lineHeight: 1.12, fontWeight: 700, marginBottom: 10 }}>
                Timeless{" "}
                <span style={{ position: "relative", display: "inline-block" }}>
                  <span style={{ position: "relative", zIndex: 1 }}>Togetherness</span>
                  <span style={{ position: "absolute", bottom: 4, left: -4, right: -4, height: 14, background: `${C.siteBrown}20`, borderRadius: 4, zIndex: 0 }} />
                </span>
              </h1>
              <p style={{ fontSize: 18, lineHeight: 1.7, color: C.siteTextMuted, marginTop: 20, maxWidth: 520 }}>
          ...
          {/* Stats strip */}
          <div style={{ display: "flex", gap: 40, marginTop: 56, flexWrap: "wrap", padding: "32px 0", borderTop: `1px solid ${C.siteWarmGray}` }}>
     ...
     {/* ═══════════ IMAGE CAROUSEL ═══════════ */}
      <ImageCarousel />
       {/* ═══════════ ABOUT ═══════════ */}
      <section id="about" className="section-pad" style={{ padding: "100px 0", background: C.siteWhite }}>
        <div style={px}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 8, marginBottom: 44, flexWrap: "wrap" }}>
            {ABOUT_TABS.map(t => (
              <button key={t.key} onClick={() => setActiveTab(t.key)} style={{
                ...
                background: activeTab === t.key ? C.siteGreen : `${C.siteWarmGray}60`,
                color: activeTab === t.key ? C.siteCream : C.siteBrown, transition: "all 0.3s ease",
              }}>{t.label}</button>
            ))}
          </div>
            ...
            <FadeIn>
              <div style={{ maxWidth: 860, margin: "0 auto" }}>
                <div style={{ marginBottom: 48 }}>
                  <h3 style={{ fontFamily: FONTS.serif, fontSize: 24, color: C.siteGreen, marginBottom: 16, display: "flex", alignItems: "center", gap: 12 }}>
                    <span style={{ width: 36, height: 36, borderRadius: "50%", background: `${C.siteGreen}10`, display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><span style={{ fontSize: 18 }}>🌿</span></span>
                    The Gap We Bridge
                  </h3>
                  <p style={{ fontSize: 16, lineHeight: 1.8, color: C.siteTextMuted, paddingLeft: 48 }}>For too long, society has moved in separate circles. We've placed our experience on the sidelines and our energy at the core. At Saathban, we believe that a community is only as strong as its connections. We are the bridge between the keepers of our legacy and the leaders of our future.</p>
                </div>
                <div style={{ marginBottom: 48 }}>
                  <h3 style={{ fontFamily: FONTS.serif, fontSize: 24, color: C.siteGreen, marginBottom: 16, display: "flex", alignItems: "center", gap: 12 }}>
                    <span style={{ width: 36, height: 36, borderRadius: "50%", background: `${C.siteBrown}10`, display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><span style={{ fontSize: 18 }}>🤝</span></span>
                    Beyond Care, Toward Contribution
                  </h3>
                  <p style={{ fontSize: 16, lineHeight: 1.8, color: C.siteTextMuted, paddingLeft: 48 }}>We are a platform that celebrates age as it tends to bring about contentment, wisdom and peace. By creating a space for <strong style={{ color: C.siteGreen }}>Saath-Icons</strong> (our elders) and <strong style={{ color: C.siteBrown }}>Saath-Buddies</strong> (our youth) to interact, we transform quiet moments into shared milestones.</p>
                </div>
                <div style={{ background: `linear-gradient(135deg, ${C.siteGreen}08, ${C.siteBrown}06)`, borderRadius: 20, padding: "36px 40px", border: `1px solid ${C.siteWarmGray}50` }}>
                  <h3 style={{ fontFamily: FONTS.serif, fontSize: 24, color: C.siteGreen, marginBottom: 16, display: "flex", alignItems: "center", gap: 12 }}>
                    <span style={{ width: 36, height: 36, borderRadius: "50%", background: `${C.siteSage}20`, display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><span style={{ fontSize: 18 }}>🌳</span></span>
                    A Timeless Society
                  </h3>
                  <p style={{ fontSize: 16, lineHeight: 1.8, color: C.siteTextMuted, paddingLeft: 48 }}>Inspired by the strength of a rooted tree and the warmth of a rising sun, Saathban is building a seamless reality where age is a badge of honor, and togetherness is our natural state.</p>
                </div>
              </div>
            </FadeIn>
          )}
            ...
            <FadeIn>
              <div style={{ maxWidth: 960, margin: "0 auto" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 36 }} className="mission-grid">
                  {FOUNDERS.map((f, i) => (
                    <div key={i} style={{ background: C.siteBg, borderRadius: 20, overflow: "hidden", boxShadow: "0 2px 20px rgba(6,50,20,0.06)", border: `1px solid ${C.siteWarmGray}50` }}>
                      {/* Photo */}
                      <div style={{ paddingTop: 32, display: "flex", justifyContent: "center", background: `linear-gradient(135deg, ${C.siteGreen}12, ${C.siteBrown}08)`, }}>
                        <div style={{ width: 180, height: 180, borderRadius: "50%", overflow: "hidden", border: `3px solid ${i === 0 ? C.siteGreen : C.siteBrown}25`, background: i === 0 ? `${C.siteGreen}10` : `${C.siteBrown}10`, display: "flex", alignItems: "center", justifyContent: "center", position: "relative", }}>
                          <img
                            src={f.img}
                            alt={f.name}
                          ...
                          <div style={{ display: "none", position: "absolute", inset: 0, alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 8, }}>
                            <span
                              style={{ fontFamily: FONTS.serif, fontSize: 32, fontWeight: 700, color: i === 0 ? C.siteGreen : C.siteBrown, }}>
                      ...
                      <div style={{ padding: 32 }}>
                        <h3 style={{ fontFamily: FONTS.serif, fontSize: 22, color: C.siteGreen, marginBottom: 4 }}>{f.name}</h3>
                        <span style={{ fontSize: 12, fontWeight: 700, color: i === 0 ? C.siteGreen : C.siteBrown, background: i === 0 ? `${C.siteGreen}10` : `${C.siteBrown}10`, padding: "4px 14px", borderRadius: 20, textTransform: "uppercase", letterSpacing: "0.06em", display: "inline-block", marginBottom: 16 }}>{f.role}</span>
                        <p style={{ fontSize: 15, lineHeight: 1.75, color: C.siteTextMuted, marginBottom: 20 }}>{f.bio}</p>
                        {/* Message */}
                        <div style={{ background: i === 0 ? `${C.siteGreen}06` : `${C.siteBrown}06`, borderLeft: `3px solid ${i === 0 ? C.siteGreen : C.siteBrown}`, borderRadius: "0 12px 12px 0", padding: "16px 20px" }}>
                          <div style={{ fontSize: 28, color: i === 0 ? C.siteGreen : C.siteBrown, opacity: 0.25, fontFamily: FONTS.quote, lineHeight: 1, marginBottom: 6 }}>"</div>
                          <p style={{ fontFamily: FONTS.serif, fontSize: 15, fontStyle: "italic", color: i === 0 ? C.siteGreen : C.siteBrown, lineHeight: 1.7 }}>{f.message}</p>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          {activeTab === "mission" && (
            <FadeIn>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, alignItems: "center" }} className="mission-grid">
                <div>
                  <h3 style={{ fontFamily: FONTS.serif, fontSize: 26, color: C.siteGreen, marginBottom: 18 }}>Our Mission</h3>
                  <p style={{ fontSize: 16, lineHeight: 1.8, color: C.siteTextMuted }}>We are working toward dismantling the barriers of age by facilitating purposeful interactions between generations. By creating structured opportunities for mentorship and shared experiences, we ensure that every individual remains an active contributor to the social, emotional, and cultural wealth of our society.</p>
                </div>
                <div style={{ background: `linear-gradient(135deg, ${C.siteGreen}08, ${C.siteBrown}06)`, borderRadius: 20, padding: 40, display: "flex", flexDirection: "column", gap: 20, border: `1px solid ${C.siteWarmGray}50` }}>
                  {["Dismantle barriers of age through purposeful interaction", "Facilitate mentorship between Saath-Icons & Saath-Buddies", "Create structured opportunities for shared experiences", "Ensure every individual contributes to our collective wealth"].map((item, i) => (
                    <div key={i} style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
                      <div style={{ width: 28, height: 28, borderRadius: "50%", background: `${[C.siteGreen,C.siteBrown,C.siteSage,C.siteOlive][i]}15`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 2 }}>
                        <div style={{ width: 8, height: 8, borderRadius: "50%", background: [C.siteGreen,C.siteBrown,C.siteSage,C.siteOlive][i] }} />
                      </div>
                      <span style={{ fontSize: 15, color: C.siteTextMain, lineHeight: 1.6 }}>{item}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {activeTab === "vision" && (
            <FadeIn>
              <div style={{ maxWidth: 720, margin: "0 auto", textAlign: "center" }}>
                ...
                <h3 style={{ fontFamily: FONTS.serif, fontSize: 26, color: C.siteGreen, marginBottom: 20 }}>Our Vision</h3>
                <p style={{ fontSize: 18, lineHeight: 1.8, color: C.siteTextMuted, marginBottom: 24 }}>To redefine aging as a period of renewed status and social vitality. We imagine a future where the artificial divide between young and old is gone, replaced by a seamless society that treasures experience as much as energy and views togetherness as our greatest strength.</p>
                <div style={{ background: `linear-gradient(135deg, ${C.siteGreen}10, ${C.siteSage}15)`, borderRadius: 16, padding: "28px 36px", display: "inline-block", marginTop: 12 }}>
                  <p style={{ fontFamily: FONTS.serif, fontSize: 20, fontStyle: "italic", color: C.siteGreen, lineHeight: 1.6 }}>"Age is a badge of honor, and togetherness is our natural state."</p>
                </div>
              </div>
            </FadeIn>
          )}
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 24, maxWidth: 960, margin: "0 auto", justifyContent: "center" }}>
                {TEAM.map((m, i) => (
                  <Card key={m.name} style={{ textAlign: "center", padding: 28, flex: "1 1 200px", maxWidth: 260 }}>
                    ...
                    <h4 style={{ fontSize: 16, fontWeight: 700, color: C.siteGreen, marginBottom: 4 }}>{m.name}</h4>
                    <p style={{ fontSize: 13, color: C.siteBrown, fontWeight: 600, letterSpacing: "0.02em" }}>{m.role}</p>
                  </Card>
                ))}
              </div>
            </FadeIn>
      ...
      {/* ═══════════ OUR WORK ═══════════ */}
      <section id="work" className="section-pad" style={{ padding: "100px 0", background: C.siteBg }}>
        <div style={px}>
          <SecTitle sub="From grassroots events to research — here's how we make a difference.">Our Work</SecTitle>

          {/* Reports Carousel */}
          <FadeIn>
            <h3 style={{ fontFamily: FONTS.serif, fontSize: 22, color: C.siteGreen, marginBottom: 24, display: "flex", alignItems: "center", gap: 12 }}>
              <span style={{ width: 36, height: 36, borderRadius: 10, background: `${C.siteGreen}10`, display: "inline-flex", alignItems: "center", justifyContent: "center" }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={C.siteGreen} strokeWidth="2"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>
              </span>
              Research & Reports
            </h3>
            <div style={{ marginBottom: 56 }}>
              <Carousel items={RESEARCH} perPage={2} renderCard={(r, i) => (
                <Card key={r.title} style={{ borderLeft: `4px solid ${r.tagColor}`, height: "100%" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                    <h4 style={{ fontSize: 17, fontWeight: 700, color: C.siteGreen, flex: 1, lineHeight: 1.4 }}>{r.title}</h4>
                  ...
                  <p style={{ fontSize: 14, color: C.siteTextMuted, lineHeight: 1.65, marginBottom: 16 }}>{r.summary}</p>
                  {r.link ? (
                    ...
                    <span style={{ fontSize: 13, color: C.siteTextMuted, fontStyle: "italic" }}>Full report coming soon</span>
                  )}
                </Card>
              )} />
            </div>
          ...
          {/* Events Carousel */}
          <FadeIn delay={0.1}>
            <h3 style={{ fontFamily: FONTS.serif, fontSize: 22, color: C.siteGreen, marginBottom: 8, display: "flex", alignItems: "center", gap: 12 }}>
              <span style={{ width: 36, height: 36, borderRadius: 10, background: `${C.siteBrown}10`, display: "inline-flex", alignItems: "center", justifyContent: "center" }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={C.siteBrown} strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
              </span>
              Events
            </h3>
            <p style={{ fontSize: 14, color: C.siteTextMuted, marginBottom: 24, marginLeft: 48 }}>Click on a past event to view photos, highlights, and full details.</p>
            <Carousel items={EVENTS} perPage={3} renderCard={(ev, i) => (
              <Card key={ev.id} style={{ overflow: "hidden", position: "relative", height: "100%" }}
                ...
                <span style={{ position: "absolute", top: 22, right: 20, fontSize: 11, fontWeight: 700, color: ev.detail ? C.siteBrown : C.siteGreen, background: ev.detail ? `${C.siteBrown}12` : `${C.siteGreen}10`, padding: "3px 10px", borderRadius: 20, textTransform: "uppercase", letterSpacing: "0.05em" }}>
                ...
                <h4 style={{ fontSize: 18, fontWeight: 700, color: C.siteGreen, margin: "8px 0", lineHeight: 1.35 }}>{ev.title}</h4>
                <p style={{ fontSize: 13, color: C.siteTextMuted, marginBottom: 12, display: "flex", alignItems: "center", gap: 6 }}>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={C.siteTextMuted} strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
                  {ev.loc}
                </p>
                <p style={{ fontSize: 14, color: C.siteTextMuted, lineHeight: 1.6, marginBottom: ev.detail ? 16 : 0 }}>{ev.desc}</p>
                {ev.detail && <span style={{ fontSize: 13, fontWeight: 600, color: C.siteBrown, display: "inline-flex", alignItems: "center", gap: 6 }}>View Event Details →</span>}
              </Card>
            )} />
          </FadeIn>
        </div>
      ...
      {/* ═══════════ GET INVOLVED ═══════════ */}
      <section id="involve" style={{ padding: "100px 0", background: `linear-gradient(135deg, ${C.siteGreen}, ${C.siteGreenLight})`, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", bottom: -50, left: -50, width: 250, height: 250, borderRadius: "50%", background: `${C.siteBrown}10` }} />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 28, maxWidth: 860, margin: "0 auto" }} className="mission-grid">
            <FadeIn delay={0.1}>
              <div style={{ background: "rgba(250,243,233,0.08)", backdropFilter: "blur(12px)", borderRadius: 20, padding: 40, border: "1px solid rgba(250,243,233,0.12)", height: "100%" }}>
                <div style={{ width: 52, height: 52, borderRadius: 14, background: `${C.siteCream}15`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 20 }}>
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={C.siteCream} strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
                </div>
                <h3 style={{ fontFamily: FONTS.serif, fontSize: 24, color: C.siteCream, marginBottom: 14 }}>Become a Saath-Buddy</h3>
            ...
            <FadeIn delay={0.2}>
              <div style={{ background: "rgba(250,243,233,0.08)", backdropFilter: "blur(12px)", borderRadius: 20, padding: 40, border: "1px solid rgba(250,243,233,0.12)", height: "100%" }}>
                <div style={{ width: 52, height: 52, borderRadius: 14, background: `${C.siteCream}15`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 20 }}>
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={C.siteCream} strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
                </div>
                <h3 style={{ fontFamily: FONTS.serif, fontSize: 24, color: C.siteCream, marginBottom: 14 }}>Partner With Us</h3>
      ...
      {/* ═══════════ BLOG / STORIES ═══════════ */}
      <section id="blog" className="section-pad" style={{ padding: "100px 0", background: C.siteWhite }}>
        <div style={px}>
          ...
          <Carousel items={BLOGS} perPage={3} renderCard={(b, i) => {
            const tc = tagColors[b.tag] || C.siteGreen;
            return (
              <Card key={b.id} style={{ display: "flex", flexDirection: "column", height: "100%", padding: 0, overflow: "hidden", cursor: "pointer" }} onClick={() => goToBlog(b.id)}>
                ...
                <div style={{ height: 180, position: "relative", overflow: "hidden", flexShrink: 0, background: `linear-gradient(135deg, ${tc}15, ${tc}05)` }}>
                  ...
                  {/* Tag pill — top left */}
                  <span style={{ position: "absolute", bottom: 14, left: 14, fontSize: 11, fontWeight: 700, color: C.siteWhite, background: tc, padding: "4px 12px", borderRadius: 20, textTransform: "uppercase", letterSpacing: "0.06em", backdropFilter: "blur(4px)" }}>{b.tag}</span>
                  {/* Read time — top right */}
                  <span style={{ position: "absolute", bottom: 14, right: 14, fontSize: 11, fontWeight: 600, color: C.siteWhite, background: "rgba(0,0,0,0.35)", padding: "4px 10px", borderRadius: 20, backdropFilter: "blur(4px)" }}>{b.readTime}</span>
                </div>
                {/* Card content */}
                <div style={{ padding: "20px 24px 24px", flex: 1, display: "flex", flexDirection: "column" }}>
                  <span style={{ fontSize: 12, color: C.siteTextMuted, fontWeight: 500, marginBottom: 8 }}>{b.date}</span>
                  <h4 style={{ fontSize: 17, fontWeight: 700, color: C.siteGreen, lineHeight: 1.4, marginBottom: 10 }}>{b.title}</h4>
                  <p style={{ fontSize: 14, color: C.siteTextMuted, lineHeight: 1.6, flex: 1 }}>{b.excerpt}</p>
          ...
          {/* Newsletter */}
          <FadeIn delay={0.2}>
            <div style={{ marginTop: 64, background: `linear-gradient(135deg, ${C.siteGreen}08, ${C.siteBrown}06)`, borderRadius: 20, padding: "48px 40px", textAlign: "center", border: `1px solid ${C.siteWarmGray}50`, maxWidth: 680, marginInline: "auto" }}>
              <h3 style={{ fontFamily: FONTS.serif, fontSize: 24, color: C.siteGreen, marginBottom: 10 }}>Stay Connected</h3>
              <p style={{ fontSize: 15, color: C.siteTextMuted, marginBottom: 28, lineHeight: 1.6 }}>Subscribe to our newsletter for updates on research, events, and stories from the Saathban community.</p>
              {subbed ? (
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, color: C.siteGreen, fontWeight: 600, fontSize: 16 }}>
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={C.siteGreen} strokeWidth="2.5"><path d="M20 6L9 17l-5-5"/></svg>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12, alignItems: "center" }}>
                  <div style={{ display: "flex", gap: 12, maxWidth: 440, width: "100%", flexWrap: "wrap", justifyContent: "center" }}>
                    <input type="email" placeholder="Your email address" value={email} onChange={e => setEmail(e.target.value)}
                      style={{ flex: "1 1 240px", padding: "14px 20px", borderRadius: 50, border: `1.5px solid ${C.siteWarmGray}`, fontSize: 15, fontFamily: FONTS.sans, background: C.siteWhite, color: C.siteTextMain, minWidth: 200 }} />
                    <Btn onClick={async () => {
                      setSubEmailError("");
                  ...
                  {subEmailError && <p style={{ fontSize: 12, color: C.siteBrown, margin: 0 }}>{subEmailError}</p>}
                  {subError && <p style={{ fontSize: 13, color: C.siteBrown, margin: 0 }}>Something went wrong — email us at <a href="mailto:hr@saathban.com" style={{ color: C.siteBrown }}>hr@saathban.com</a></p>}
                </div>
              )}
            </div>
          </FadeIn>
      ...
      {/* ═══════════ CONTACT ═══════════ */}
      <section id="contact" className="section-pad" style={{ padding: "100px 0", background: C.siteBg }}>
        <div style={px}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 28, maxWidth: 920, margin: "0 auto", alignItems: "start" }} className="mission-grid">
            <FadeIn>
              <div>
                <h3 style={{ fontFamily: FONTS.serif, fontSize: 22, color: C.siteGreen, marginBottom: 24 }}>Reach Out</h3>
                <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                  {[
                    { href: `mailto:${SOCIAL_LINKS.email}`, label: "Email", value: SOCIAL_LINKS.email, color: C.siteGreen, icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={C.siteGreen} strokeWidth="2"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M22 4l-10 8L2 4"/></svg> },
                    { href: SOCIAL_LINKS.instagram, label: "Instagram", value: "@saathban", color: C.siteBrown, icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={C.siteBrown} strokeWidth="2"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="5"/><circle cx="17.5" cy="6.5" r="1.5" fill={C.siteBrown}/></svg> },
                    { href: SOCIAL_LINKS.facebook, label: "Facebook", value: "Saathban", color: C.siteGreen, icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={C.siteGreen} strokeWidth="2"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/></svg> },
                    { href: SOCIAL_LINKS.linkedin, label: "LinkedIn", value: "Saathban", color: C.siteGreen, icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={C.siteGreen} strokeWidth="2"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-4 0v7h-4v-7a6 6 0 0 1 6-6z"/><rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/></svg> },
                    ...
                    <a key={label} href={href} target={href.startsWith("mailto") ? undefined : "_blank"} rel="noopener noreferrer" style={{ display: "flex", gap: 16, alignItems: "center", textDecoration: "none" }}>
                      ...
                      <div>
                        <div style={{ fontSize: 12, color: C.siteTextMuted, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 2 }}>{label}</div>
              ...
              <Card>
                {sent ? (
                  <div style={{ textAlign: "center", padding: "20px 0" }}>
                    <div style={{ width: 56, height: 56, borderRadius: "50%", background: `${C.siteGreen}12`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
                      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={C.siteGreen} strokeWidth="2.5"><path d="M20 6L9 17l-5-5"/></svg>
                    </div>
                    <h4 style={{ fontSize: 20, fontWeight: 700, color: C.siteGreen, marginBottom: 8 }}>Message Sent!</h4>
                    <p style={{ fontSize: 14, color: C.siteTextMuted }}>We'll get back to you soon at {contact.email}.</p>
                    <span onClick={() => { setSent(false); setContact({ name: "", email: "", message: "", contactType: "General" }); }}
                      style={{ fontSize: 13, color: C.siteBrown, cursor: "pointer", marginTop: 12, display: "inline-block", fontWeight: 600 }}>Send another message</span>
                  </div>
                ) : (
                  <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <h4 style={{ fontSize: 18, fontWeight: 700, color: C.siteGreen }}>Send Us a Message</h4>
                      <div style={{ display: "flex", gap: 6 }}>
                        {["General", "Volunteer", "Partner"].map(t => (
                          <span key={t} onClick={() => setContact(c => ({ ...c, contactType: t }))}
                            style={{ fontSize: 11, fontWeight: 700, padding: "4px 12px", borderRadius: 20, cursor: "pointer", background: contact.contactType === t ? C.siteGreen : `${C.siteWarmGray}60`, color: contact.contactType === t ? C.siteCream : C.siteTextMuted, transition: "all 0.2s ease" }}>{t}</span>
                        ))}
                      </div>
                    </div>
                    <input placeholder="Your Name" value={contact.name} onChange={e => setContact({ ...contact, name: e.target.value })}
                      style={{ padding: "13px 18px", borderRadius: 12, border: `1.5px solid ${C.siteWarmGray}`, fontSize: 15, fontFamily: FONTS.sans, background: C.siteBg }} />
                    <input type="email" placeholder="Your Email" value={contact.email} onChange={e => setContact({ ...contact, email: e.target.value })}
                      style={{ padding: "13px 18px", borderRadius: 12, border: `1.5px solid ${emailError ? C.siteBrown : C.siteWarmGray}`, fontSize: 15, fontFamily: FONTS.sans, background: C.siteBg }} />
                    {emailError && <p style={{ fontSize: 12, color: C.siteBrown, marginTop: -10, marginBottom: -4 }}>{emailError}</p>}
                    <textarea placeholder="Your Message" rows={4} value={contact.message} onChange={e => setContact({ ...contact, message: e.target.value })}
                      style={{ padding: "13px 18px", borderRadius: 12, border: `1.5px solid ${C.siteWarmGray}`, fontSize: 15, fontFamily: FONTS.sans, background: C.siteBg, resize: "vertical" }} />
                    <Btn onClick={async () => {
                      setEmailError("");
      ...
      {/* ═══════════ FOOTER ═══════════ */}
      <footer style={{ background: C.siteGreen, color: "rgba(250,243,233,0.7)", padding: "60px 0 32px" }}>
        <div style={px}>
          <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 40, marginBottom: 40, paddingBottom: 32, borderBottom: "1px solid rgba(250,243,233,0.1)" }}>
            ...
            <div>
              <h4 style={{ fontSize: 13, fontWeight: 700, color: C.siteCream, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>Quick Links</h4>
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {NAV_ITEMS.map(n => (
                  <span key={n.id} onClick={() => scrollTo(n.id)} style={{ cursor: "pointer", fontSize: 14, transition: "color 0.3s" }}
                    onMouseEnter={e => e.target.style.color = C.siteCream} onMouseLeave={e => e.target.style.color = "rgba(250,243,233,0.7)"}>{n.label}</span>
                ))}
              </div>
            </div>
            <div>
              <h4 style={{ fontSize: 13, fontWeight: 700, color: C.siteCream, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>Connect</h4>
              <div style={{ display: "flex", gap: 12 }}>
                <SocialIcon href={`mailto:${SOCIAL_LINKS.email}`} icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={C.siteCream} strokeWidth="2"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M22 4l-10 8L2 4"/></svg>} />
                <SocialIcon href={SOCIAL_LINKS.instagram} icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={C.siteCream} strokeWidth="2"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="5"/></svg>} />
                <SocialIcon href={SOCIAL_LINKS.facebook} icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={C.siteCream} strokeWidth="2"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/></svg>} />
                <SocialIcon href={SOCIAL_LINKS.linkedin} icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={C.siteCream} strokeWidth="2"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-4 0v7h-4v-7a6 6 0 0 1 6-6z"/><rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/></svg>} />
              </div>
            </div>
          </div>
