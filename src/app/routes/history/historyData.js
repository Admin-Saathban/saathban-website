...
export function moduleSummary(rows, monthKey) {
  ...
  for (const [module, list] of Object.entries(byModule)) {
    ...
    if (module === "sleep") {
      ...
      stat = { kind: "meals", value: list.reduce((n, r) => n + (r.payload?.answers && typeof r.payload.answers === "object"
        ? Object.values(r.payload.answers).filter((a) => a && a.had === true).length
        : r.payload?.meals?.length || Object.values(r.payload?.entries || {}).flat().length || 0), 0) };
    } else if (module === "mood") {
