...
| outdoor / groups (second block) | 0110–0119 |
...
| 0065 | integration | `0065_access_notes_verified.sql` — **applied**: OUT_AND_ABOUT_SPEC §4.1, now ruled (admin-seeded launches; admins edit later). Adds `verified`/`verified_by`/`verified_at`, and the consequence that matters: **an unverified note is withheld from public place rows entirely**. The notes seeded in 0064 were mine and are guesses — I have not stood at the gate of Hill Park — and §4 is blunt about the cost: "if it says 'flat walk' and there are steps, someone made a trip they could not complete." A chip cannot say "probably", so a guess shown in the same green as a checked fact IS that harm and no wording fixes it. The deliberate consequence is that seeded places show NO chips until an admin confirms them on `/app/admin/places`, which is a few taps per place. The backfill sets `verified = false` explicitly rather than relying on the column default, so re-running cannot quietly promote a guess to a fact. Verified: an Icon is refused a write (403), an unchecked note never reaches the place query, the SAME note reaches it once confirmed, and a place's checked note does not drag its unchecked ones into view — the last one catching a filter written per place instead of per row | 2026-08-30 |
| 0066 | integration | `0066_moments.sql` — **applied**: OUT_AND_ABOUT_SPEC §8 "I'm at X". Not a check-in and not a place: §4.1 makes places admin-seeded, so without this the only ways to say "I'm at the chai stall on the corner" are to petition for a permanent civic record of a chai stall, or to say nothing. Three windows — live (ordinary widening rules), past (**only the author and the people who were there**, which is what `outdoor_moment_presence` exists to answer), gone 48h after it started **for everyone including the author** — and **all three are in the read policy, not a cleanup job**: a job that fails to run leaves somebody's movements readable indefinitely and fails silently. Deletion is deliberately not immediate because §8's window exists "to report anything". Presence can only be claimed while a moment is LIVE, or joining a finished one would be a way to buy the right to read it. Verified: 8 checks, each refusal paired with a control — live reaches others, a non-attendee is refused once it ends, an attendee still reads it, a late join is refused (403), a client cannot back-date its own row out of the window (403), anon 401; the 48h cut checked separately with rows seeded owner-side and read back as their own author, since the back-date refusal means a test cannot age a row either | 2026-08-30 |
| 0067 | integration | `0067_group_interior.sql` — **applied**: GROUPS_SPEC §3/§6/§7/§8. `group_members.role` gains `co_admin` (§7.3) — and `is_group_creator()` is deliberately **left alone**, because §7.3 reserves deleting the group and handing over ownership to the owner; folding admins into that predicate would hand a co-admin the power to delete somebody's group, the invisible-widening mistake 0053 documents. A co-admin also cannot promote another co-admin, or the first one appoints the rest and the owner is no longer the owner. §8's pin is **one per group by partial unique index**, not by RPC logic alone, since "at most one" enforced in a function is one concurrent call away from being two; `pin_group_post` unpins first so "pin this instead" works. §6's `group_post_in_main_feed()` is a **composition rule, not a permission rule** — a member may still READ their private group's posts, so this can only break by the feed asking the wrong question and would look correct in every RLS test ever written; hence a named predicate. Verified: 6 checks including that a member can read both groups (the control that makes the rest meaningful), private never reaches the feed, a public group you have NOT joined stays out, the pin moves rather than duplicating, and a non-admin cannot pin | 2026-08-30 |
| 0068 | integration | `0068_is_group_admin_reconciled.sql` — **applied**: two lanes defined `is_group_admin(uuid)` within minutes, mine (0067: creator OR co_admin) and Lane 2's (0086: creator OR platform admin). **Same signature, so this is NOT the 0049 overload trap — `create or replace` does not collide on an identical signature, it silently REPLACES.** Mine applied second, so Lane 2's join-request SELECT policy and both their RPCs, all of which route through this one function, quietly stopped admitting platform admins. That shape is more dangerous than the overload: an overload fails loudly on the next call, whereas this narrowed a permission predicate underneath code already verified against it — verification does not stick when a shared predicate can be redefined out from under it. The fix is the **union**, since both call sites wanted exactly that and neither lane changes a line: `is_admin() OR role in ('creator','co_admin')`. Found by Lane 2 reporting a broken build, not by either of us testing | 2026-08-30 |
| 0069 | integration | `0069_an_owner_can_close_a_group.sql` — **applied**: GROUPS_SPEC §7.3, which was never implemented. `groups` had a read policy and an admin update policy and **nothing for delete**, so not even the person who started a group could close it. Adds `delete_group` and `transfer_group_ownership`, both gated on `is_group_creator` and pointedly **NOT** `is_group_admin` — a co-admin helps run a group, they do not get to end it, and a platform admin already has `hidden_at` for moderation, which is reversible and leaves the evidence. Handing over requires the recipient to be a member (nobody is made responsible for a room they have never been in) and leaves the old owner as a co-admin rather than silently demoting them to a member. **How it was found matters more than the fix:** my test fixtures deleted their groups through a REST DELETE whose result nobody read — with no delete policy the statement matches zero rows and returns **204**, a success shape for an operation that did nothing. 47 fixture groups accumulated at one per run, and because they are `privacy = 'anyone'` they became the entire contents of "Groups near you" on the search screen; a peer found real users being offered S7TEST and DUPTEST. The missing policy silently produced a product gap AND a data leak into a user-facing list, and the test that should have caught it was the thing generating it. All three group suites now sweep through `delete_group` and **assert the sweep worked**, exiting 3 if anything survives. Verified: a co-admin is refused, a hand-over to a non-member is refused, the group survives both refusals, the owner can close it, and it is actually gone rather than merely reported gone | 2026-08-30 |
| 0110 | outdoor / groups | `0110_group_covers.sql` — **applied**: GROUPS_SPEC §1/§3 covers and the finish-setting-up row. `groups.cover` holds EITHER `preset:<type>` or a storage path, in one column, because "which kind of cover is this" is never asked separately from "what is the cover". The preset matters more than the upload: §1 keeps cover and description out of creation because "older users abandon at the photo step", and says the type "gives a default cover image, so nobody has to find a photo" — a cover feature whose only path is upload would put that step straight back, just later. So every group has a finished cover from the moment it exists. The `group-covers` bucket is **private**: a private group's cover is a photograph of a private group, and a public bucket would put it on a guessable URL outside RLS entirely (the CNIC rule from SPEC.md). Storage policies read the FIRST PATH SEGMENT as the group id — `<group_id>/<file>` — so `can_see_group` gates reads and `is_group_admin` gates writes; changing that path shape silently changes who may write a cover. `set_group_cover` and `dismiss_group_setup` are RPCs rather than plain updates because `groups` has an admin-only update policy, so an owner's UPDATE is refused — the same silent-refusal shape that let 47 fixture groups accumulate. `setup_dismissed_at` belongs to the GROUP, not the person: the row is about the group being unfinished, and a second owner should not be nagged about a decision the first already declined | 2026-08-31 |
...
| 0118 | integration | `0118_a_survey_is_answered_once.sql` — **applied**: the research survey could be "submitted" repeatedly. 0054 already made profile_id UNIQUE and that is live, so duplicate ROWS were never possible and none exist; what happened was an OVERWRITE — the client upserts on profile_id and the update policy let each new submission replace the last answers. A BEFORE UPDATE trigger now makes answers, submitted_at and profile_id final once submitted_at is set, for every role, and refuses out loud rather than matching zero rows. Delete stays open: withdrawal is the consent screen's "stop at any point", and after it one fresh row may be written. Verified live as a submitted Icon (the client's upsert and a direct PATCH are both refused) with a control on a second account whose unsubmitted row updates, submits, is then refused, and deletes | 2026-09-12 |
| 0119 | integration | `0119_meals_and_movement_keep_their_names.sql` — **applied**: (1) daily_log_prefs gains meal_categories and movement_options, [{id,key,name}], NULL = never set = the defaults (Breakfast/Lunch/Dinner; A walk/Stretching/Gardening/Housework/Something else); no UPDATE, meal_items kept. (2) A BEFORE INSERT/UPDATE trigger on daily_logs makes a record keep the name of what was entered, for every build: a ticked food without a name gets it from the list at save time; names already in a row are carried forward so no writer can drop them; a food with no name anywhere is REFUSED, except ids already nameless before this rule (grandfathered) and the old fixed slot ids; a meal answer or movement activity must carry a name or a default key; an old writer sending only type keeps the activity. Measured first: 9 of 22 ticked foods already nameless, all test-icon (4 old slot ids with known names, 4 foods permanently lost; free plan, no restorable backups). Verified live as test-icon with controls: name filled on insert, kept when a save omits it, unknown nameless food refused, nameless answer refused vs named accepted, nameless activity and unknown bare type refused vs named accepted, type-only save keeps the activity, the 12 Sep row with lost foods saves back unchanged; test rows deleted | 2026-09-13 |
| 0120 | integration | `0120_sharing_shows_who_and_what.sql` — **applied**: sharing shows what goes out and to whom before it is sent. (1) `share_audience(p_audience)` returns only the first names a share would reach ('circle', 'friends' = accepted conversations, 'connections'), limit 50, for the caller only. (2) `share_score_to_audience(p_audience, p_title, p_body, p_link)` sends the person's OWN edited words as 'score' notifications to 'circle' or 'friends' separately (before, both rows reached the same combined audience) and returns how many were actually notified after notify_allowed. (3) `boast_to_people_worded(p_kind, p_ref, p_payload, p_title, p_body)` — the riddle "tell your people" with words written in the language in use (the older boast_to_people wrote English on the server); still once per kind+ref through `boasts`, returns rows written. (4) `has_boasted(p_kind, p_ref)`. Functions only: no tables, no data changed, older RPCs left in place | 2026-09-12 |
| 0121 | integration | `0121_a_riddle_helps_one_step_at_a_time.sql` — **applied**: daily_puzzles gains `hints_en text[]` and `hints_ur text[]` (1-5 each, check constraint `daily_puzzles_hints_shape`) and all 30 seeded riddles (2026-08-29..2026-09-27) get a three-step ladder in both languages — a nudge, closer, then a plain description with the first letter, never the answer itself. Read with the riddle under the existing policy (up to today only); daily_puzzle_answers untouched and still unreadable. hint_en/hint_ur left as they were for clients that have not reloaded. Hints are never sent to the server and change nothing in puzzle_attempts. **Content note: no riddle exists after 2026-09-27** | 2026-09-12 |
| 0122 | integration | `0122_safe_profiles_cannot_be_written.sql` — **applied**: SECURITY. safe_profiles (owned by postgres, no security_invoker, auto-updatable) carried INSERT/UPDATE/DELETE for `authenticated`; proved in a rolled-back transaction that one signed-in account could UPDATE and DELETE another's profile through it. Revokes every privilege but SELECT. Nothing in the app wrote through the view | 2026-09-13 |
| 0123 | integration | `0123_people_are_seen_through_a_reason.sql` — **applied**: SECURITY. `profile_related(uuid)` (circle + circle requests, friends + requests, conversations + requests, shared groups, group invites/join requests, shared tables, game invites, people you blocked), `profile_cards(uuid[])` (name, photo, city, role by id, max 200; never presence/about), `search_people(text,text,int)` (3+ letters, start of a word in the name, max 20, no city, no admins except the org account, blocks respected), `public_profile(uuid)` (stranger view; never presence). Execute for authenticated only | 2026-09-13 |
| 0124 | integration | `0124_safe_profiles_only_shows_your_people.sql` — **applied after the client moved onto 0123**: SECURITY. The view returned every non-blocked account to any signed-in person, with last_seen_at regardless of show_presence. Now: own row, `profile_related` people, or everyone for `can_moderate()`; last_seen_at is null unless own row or show_presence, for every viewer including admins. Dry run on live data (rolled back): test people see 3–5 of 15, support admin 15; a hidden presence read NULL for all but its owner | 2026-09-13 |
| 0125 | integration | `0125_moderators_can_moderate.sql` — **applied**: SECURITY/MODERATION. "reports: admins decide" → can_moderate(); read policies on community_posts, post_comments, park_board_messages, groups, group_posts let can_moderate() see hidden content; the five "… admin moderates" UPDATE policies DROPPED (no admin can change arbitrary fields of anyone's content); trigger `hidden_state_is_moderated` refuses any change to hidden_at/hidden_by that does not come through `moderate_content(kind, id, hide, reason, report)` (can_moderate, reason required, audits content_hidden/content_unhidden with actor, author, kind, target, report); `moderator_set_pause(profile, paused, reason)` (can_moderate; only super-admin may pause any admin-role account; protected-write bypass; audits pause_account/unpause_account) — admin_set_pause and moderator_suspend now delegate to it; storage "evidence: admins only" → can_moderate(). Verified in a rolled-back transaction as a moderator: decide, hide, pause/unpause wrote audit rows naming the actor; pausing a super-admin refused; author un-hide and super-admin body rewrite changed 0 rows | 2026-09-13 |
| 0126 | integration | `0126_hidden_presence_at_the_table.sql` — **applied**: `game_table_seats(session)` (seats of a viewable table; presence null for anyone — not a bot, not the caller — whose show_presence is off; last_seen_at not returned) and `person_in_game(profile)` (a live table only if the caller can view it and the person shows presence). game_tick/exec_game_move unchanged. Direct column reads of game_seats.presence/last_seen_at close in 0127 after the app moves | 2026-09-13 |
| 0127 | integration | `0127_seat_presence_is_not_readable.sql` — **applied after the app moved onto 0126 (206de92 deployed)**: table SELECT on game_seats revoked from authenticated/anon; column SELECT re-granted on id, session_id, seat_no, profile_id, is_bot, missed_turns, score, joined_at, left_by — presence and last_seen_at are readable only through game_table_seats (masked) or by the game functions that run as owner | 2026-09-13 |
| 0130 | speed | `0130_home_feed_in_one_request.sql` — **applied**: `home_feed(p_limit int) → jsonb`, SECURITY INVOKER (row security applies to every part): the Home/Community feed page with reactions, joins, the caller's hides, help offers, saves, follows, tags, group neighbours, connections, public-group posts, and profile_cards for everyone named. Replaces ~15 per-table client queries. Additive; execute for authenticated only. Counts checked against the old separate reads as test-icon (50 posts, 18 reactions, 3 join groups, 3 author cards) | 2026-09-13 |
| 0135 | messages | `0135_a_conversation_can_be_quietened.sql` — **applied**: `dm_muted` (per-person, per-conversation mute; RLS mine-only with a participant check); `notify_dm_message` skips muted threads; `community_reports.target_kind` + `dm_request`, `dm_conversation` (request-card Report had always failed the check) | 2026-09-13 |
| 0140 | integration | `0140_shared_streaks_and_days.sql` — **applied** (plus `streak_item_value_module_type`, the same function with `module = p_item::log_module`, folded into the file): LOCAL DAY: `local_today(profile)` from profiles.timezone (invalid names dropped by trigger `profile_timezone_valid`, Asia/Karachi when unknown); `check_daily_log_date` refuses only dates later than the latest date anywhere on earth and measures the 48h window from the person's local today; `circle_logged_today` uses local today. DAYS: `logged_days(profile_id, day)` written by trigger `on_log_mark_day` on daily_logs, backfilled, never deleted; `my_days()` {days, logged_today, today, riddle_days_solved}; `person_logged_today(profile)` for self/circle only (no numbers). STREAKS: tables `streaks` (one live per owner+item; kind range/yes_no; is_private; created_on; run_started_on; settled_through), `streak_people`, `streak_sends` (UNIQUE streak_id+recipient_id+day), `streak_rest_days` (EXCLUDE one per 7 days per streak, btree_gist), `streak_nudges` (UNIQUE streak_id+to_id+day); RLS read-own only, no write policies; internal helpers revoked from clients; RPCs `streak_people_options`, `create_streak`, `update_streak`, `set_streak_people`, `archive_streak`, `my_streaks`, `streak_send_list`, `send_streak`, `received_streaks`, `streak_window`, `reply_streak`, `streak_group`, `nudge_streak`, `use_rest_day`, `restart_streak`; notifications kind 'streak' linking /app/streak/<send_id>. community_posts post_type + days_total, streak, good_day. Verified end to end in a rolled-back transaction (smoke-icon ↔ smoke-fam) | 2026-09-13 |
| 0142 | streaks | `0142_streak_run_before_a_missed_day.sql` — **applied**: `streak_run_before_missed(p_streak)` — the run as it stood before a missed day, for the missed-day screen (my_streaks().run is already 0 or 1 by then); additive, owner-only | 2026-09-13 |
