...
| 0050 | **FREE** | Claimed then released by the Lane C lane the same day, and the reason is worth keeping: the registrar ruling that themes stay DERIVED removed the only writer C1 had, so registering the table would have left dead schema for Lane A to build to sight-unseen. Better it arrives with its first real writer (A3 stickers) than is inherited empty. Agreed shape when someone needs it: `(profile_id, kind, item_key, count)`, `check (count >= 0)`, no grant/spend/purchase RPC, gift and stake as single conserving transactions with a floor check, grants idempotent on the move id | released 2026-08-30 |
