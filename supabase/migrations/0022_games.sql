-- ============================================================================
-- 0020 — Games platform rails + Daily Riddle (games lane)
--
-- Rails: a games registry; sessions with 2–4 seats (person or bot,
-- presence active/away, per-session house_rules); APPEND-ONLY moves,
-- server-validated per game type (for race100 the server IS the dice —
-- the client sends nothing it could cheat with); invites that deep-link
-- through notifications and auto-start the moment the last seat
-- accepts; per-session chat + stickers; open tables claimable from a
-- community post.
--
-- TURN TIMING: house_rules.turn_seconds (default 60) per turn. When it
-- lapses, the bot plays that seat (game_tick(), run by pg_cron every
-- minute AND callable by any participant's client for snappy in-view
-- timing). Three consecutive missed turns flip the seat to 'away' —
-- the bot simply continues for it until the person reclaims. NEVER
-- forfeited, never removed: coming back is always one tap.
--
-- First game on the rails: race100 ("Race to 100") — each turn the
-- server rolls a die and adds it to your score; first over the target
-- (house_rules.target, default 100) wins. Second: the DAILY RIDDLE —
-- one shared bilingual riddle per day (30 seeded), answers held in a
-- table with NO select policy (guessing goes through an RPC), results
-- shareable to the community, streak-forgiving by design (we count
-- what was solved, we never count gaps).
-- ============================================================================
...
insert into public.games (key, name_en, name_ur, tagline_en, tagline_ur, kind, min_seats, max_seats) values
...
-- ----------------------------------------------------------------------------
-- Sessions, seats, moves, invites, chat
-- ----------------------------------------------------------------------------
create table public.game_sessions (
  id              uuid primary key default gen_random_uuid(),
  game_key        text not null references public.games (key),
  status          text not null default 'lobby' check (status in ('lobby', 'active', 'finished')),
  seats_total     smallint not null check (seats_total between 2 and 4),
  house_rules     jsonb not null default '{}',
  -- Per-session board state for games that need one (Ludo's piece
  -- positions live here; race100 keeps its state on the seats).
  state           jsonb not null default '{}',
  -- Spoken lobby code, same spirit as circle invites (absorbed from
  -- the ludo lane's design).
  join_code       text,
...
create index game_sessions_live_idx on public.game_sessions (status) where status = 'active';
create unique index game_sessions_lobby_code
  on public.game_sessions (join_code) where status = 'lobby' and join_code is not null;
...
create table public.game_seats (
  ...
  presence     text not null default 'active' check (presence in ('active', 'away')),
...
create index game_seats_person_idx on public.game_seats (profile_id);
...
create table public.game_chat (
  ...
  sticker    text check (sticker is null or sticker in ('👍', '😄', '🎉', '🌸', '☕', '🙏', '😂', '❤️')),
  created_at timestamptz not null default now(),
  check (body is not null or sticker is not null)
);

create index game_chat_session_idx on public.game_chat (session_id, created_at);

-- Participant / viewer helpers (SECURITY DEFINER: policies never
-- recurse through each other's RLS).
...
revoke all on public.game_chat     from anon;

-- Sessions and their seats/moves are visible to participants and
-- invitees (an invitee sees the lobby they were asked into). All
-- WRITES anywhere here go through the RPCs below — moves append-only
-- with no client insert/update/delete policies at all.
create policy "sessions: participants and invitees read" on public.game_sessions
...
-- Chat: participants only; the table stays writable while the session
-- lives (lobby chat included — it's how a table warms up).
create policy "chat: participants read" on public.game_chat
  for select using (public.is_game_participant(session_id));
create policy "chat: participants write" on public.game_chat
  for insert with check (
    sender_id = auth.uid()
    and public.is_game_participant(session_id)
    and exists (
      select 1 from public.game_sessions
      where id = session_id and status <> 'finished'
    )
  );

-- ----------------------------------------------------------------------------
-- Notifications gain a deep link (nullable; existing rows unaffected).
-- The bell renders it when present — see GAMES_WIRING.md.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- The engine. exec_game_move() is the per-game validator/executor —
-- new game types extend its CASE. For race100 the move is generated
-- entirely server-side.
-- ----------------------------------------------------------------------------

create or replace function public.exec_game_move(p_session uuid, p_seat smallint, p_by_bot boolean)
returns jsonb
...
begin
  ...
  case s.game_key
    when 'race100' then
      v_target := coalesce((s.house_rules ->> 'target')::int, 100);
      ...
      v_move := jsonb_build_object('roll', v_roll, 'score', v_score);
    else
      raise exception 'Unknown game %', s.game_key;
  end case;
  ...
  if v_score >= v_target then
    ...
    where id = p_session;
    -- A human, present seat gets a "your turn" nudge.
...
revoke execute on function public.exec_game_move(uuid, smallint, boolean) from public, anon, authenticated;

-- A person plays their own turn.
create or replace function public.play_turn(p_session uuid)
returns jsonb
language plpgsql security definer
set search_path = public, pg_temp
as $$
...
begin
  ...
  -- Playing your own turn is also how you come back: presence and the
  -- miss counter reset on any real move.
  update public.game_seats
  set presence = 'active', missed_turns = 0
  where session_id = p_session and seat_no = v_seat;

  return public.exec_game_move(p_session, v_seat, false);
end;
$$;

revoke execute on function public.play_turn(uuid) from public, anon;
grant execute on function public.play_turn(uuid) to authenticated;

-- The sweep: bot plays every due seat. Runs from pg_cron every minute
-- AND from any viewer's client (so the visible countdown resolves the
-- moment it hits zero, not at the next cron minute). Due =
--   - the seat is a bot, or marked away (no waiting), or
--   - the seat is human+active and its turn clock has lapsed
--     (house_rules.turn_seconds, default 60) → that's a missed turn;
--     the third consecutive miss flips the seat to away.
...
begin
  for s in
    ...
    where status = 'active' and (p_session is null or id = p_session)
  loop
    v_guard := 0;
    loop
      v_guard := v_guard + 1;
...
-- Coming back from away — one tap, everything as they left it.
...
-- ----------------------------------------------------------------------------
-- Lobby: create, invite, accept/decline, claim an open seat, fill
-- with bots. A session starts THE MOMENT its last seat fills — every
-- human seat is notified with a link to the board.
-- ----------------------------------------------------------------------------
...
-- Open tables (the community "Open Ludo table — 2 of 4 seats" post):
-- any eligible viewer claims the next free seat; full ⇒ start.
...
begin
  ...
  if exists (select 1 from public.game_seats where session_id = p_session and profile_id = auth.uid()) then
    return p_session; -- already seated: claiming twice is just arriving
  end if;
  select count(*) into v_taken from public.game_seats where session_id = p_session;
  if v_taken >= s.seats_total then
    raise exception 'This table has already started';
  end if;
  -- Next free seat number (invited-but-undecided seats keep theirs).
...
-- The host fills every remaining seat with bots and begins.
...
-- ----------------------------------------------------------------------------
-- Daily Riddle. One shared row per day; answers live in a table with
-- NO select policy — the only way to check is guess_daily_puzzle().
-- Streak-forgiving by construction: attempts record what WAS solved;
-- nothing anywhere counts a gap.
-- ----------------------------------------------------------------------------
...
create table public.daily_puzzle_answers (
  ...
  answers     text[] not null  -- pre-normalized: lowercase, trimmed
);
...
revoke all on public.daily_puzzle_answers from anon, authenticated; -- nobody reads answers
revoke all on public.puzzle_attempts from anon;

-- Today's (and past) riddles are community-readable; tomorrow stays
-- tomorrow's surprise.
...
-- daily_puzzle_answers: RLS on, zero policies — invisible to all clients.
...
with seed(off, r_en, r_ur, h_en, h_ur, ans) as (
  values
  (0,  'I have hands but cannot clap. What am I?', 'میرے ہاتھ ہیں مگر تالی نہیں بجا سکتا۔ میں کون ہوں؟', 'It hangs on the wall.', 'دیوار پر لٹکی رہتی ہے۔', array['clock','watch','گھڑی']),
  (1,  'The more I dry, the wetter I get. What am I?', 'جتنا سکھاؤں، اتنا بھیگتا جاؤں۔ میں کیا ہوں؟', 'You use it after a bath.', 'نہانے کے بعد کام آتا ہے۔', array['towel','تولیہ']),
  (2,  'Full of holes, yet I hold water. What am I?', 'سوراخوں سے بھرا، پھر بھی پانی تھامے۔ میں کیا ہوں؟', 'It lives near the sink.', 'برتن دھونے میں ساتھی۔', array['sponge','اسفنج','سپنج']),
  (3,  'I have a neck but no head. What am I?', 'گردن ہے مگر سر نہیں۔ میں کیا ہوں؟', 'It holds what you drink.', 'پینے کی چیز اس میں رہتی ہے۔', array['bottle','بوتل']),
  (4,  'I always go up and never come down. What am I?', 'ہمیشہ بڑھتی ہوں، کبھی گھٹتی نہیں۔ میں کیا ہوں؟', 'Every birthday adds one.', 'ہر سالگرہ پر ایک اور۔', array['age','عمر']),
  (5,  'I have teeth but never eat. What am I?', 'دانت ہیں مگر کھاتا کچھ نہیں۔ میں کیا ہوں؟', 'It tidies your hair.', 'بالوں کو سنوارتی ہے۔', array['comb','کنگھی','کنگھا']),
  (6,  'The more you take, the more you leave behind. What are they?', 'جتنے لو، اتنے پیچھے چھوڑ جاؤ۔ وہ کیا ہیں؟', 'Look down on a sandy path.', 'ریت پر پیچھے مڑ کر دیکھیں۔', array['footsteps','steps','footprints','قدم','قدموں کے نشان']),
  (7,  'It belongs to you, but others use it more. What is it?', 'ہے تو آپ کا، مگر دوسرے زیادہ استعمال کرتے ہیں۔ کیا؟', 'You answer when you hear it.', 'پکارے جانے پر مڑتے ہیں۔', array['name','my name','نام']),
  (8,  'One eye, but I cannot see. What am I?', 'ایک آنکھ ہے مگر دیکھ نہیں سکتی۔ میں کیا ہوں؟', 'It pulls a thread along.', 'دھاگہ اس کے پیچھے چلتا ہے۔', array['needle','سوئی']),
  (9,  'The more you take away, the bigger I get. What am I?', 'جتنا نکالو، اتنا بڑا ہو جاؤں۔ میں کیا ہوں؟', 'Diggers make it.', 'کھودنے سے بنتا ہے۔', array['hole','گڑھا']),
  (10, 'Always coming, never arrives. What is it?', 'ہمیشہ آنے والا ہے، کبھی آتا نہیں۔ وہ کیا ہے؟', 'It is a day.', 'ایک دن کا نام ہے۔', array['tomorrow','کل']),
  (11, 'Say my name and I am broken. What am I?', 'میرا نام لو تو میں ٹوٹ جاؤں۔ میں کیا ہوں؟', 'A very quiet thing.', 'بہت خاموش چیز۔', array['silence','خاموشی']),
  (12, 'I have a bed but never sleep, I run but never walk. What am I?', 'بستر ہے پر سوتا نہیں، دوڑتا ہے پر چلتا نہیں۔ کیا؟', 'Boats ride on it.', 'کشتیاں اس پر تیرتی ہیں۔', array['river','دریا','ندی']),
  (13, 'Lighter than a feather, yet no one can hold me long. What am I?', 'پنکھ سے ہلکی، پھر بھی دیر تک کوئی نہ روک سکے۔ کیا؟', 'You take it every moment.', 'ہر لمحہ لیتے ہیں۔', array['breath','سانس']),
  (14, 'Green coat, red heart, black seeds. What am I?', 'ہرا چولا، لال دل، کالے بیج۔ میں کیا ہوں؟', 'A summer treat.', 'گرمیوں کا تحفہ۔', array['watermelon','تربوز']),
  (15, 'I travel the world while staying in my corner. What am I?', 'دنیا گھوموں مگر اپنے کونے میں رہوں۔ میں کیا ہوں؟', 'It rides on an envelope.', 'لفافے پر سوار۔', array['stamp','ٹکٹ','ڈاک ٹکٹ']),
  (16, 'A house with no doors or windows, with golden treasure inside. What is it?', 'نہ دروازہ نہ کھڑکی، اندر سنہری خزانہ۔ وہ کیا ہے؟', 'Breakfast often starts with it.', 'ناشتے کی جان۔', array['egg','انڈا','انڈہ']),
  (17, 'I fall all winter but never get hurt. What am I?', 'سردیوں بھر گرتی ہوں، چوٹ کبھی نہیں لگتی۔ کیا؟', 'The mountains wear it.', 'پہاڑوں کی چادر۔', array['snow','برف']),
  (18, 'I have a tongue but cannot talk. What am I?', 'زبان ہے مگر بول نہیں سکتا۔ میں کیا ہوں؟', 'It walks everywhere with you.', 'ہر قدم پر ساتھ۔', array['shoe','جوتا']),
  (19, 'A white field, black seeds, read by the wise. What is it?', 'سفید کھیت، کالے بیج، پڑھے سیانا۔ وہ کیا ہے؟', 'You are holding one now, in a way.', 'علم کا گھر۔', array['book','writing','کتاب','لکھائی']),
  (20, 'I cry without eyes and fly without wings. What am I?', 'آنکھوں بغیر روؤں، پروں بغیر اڑوں۔ میں کیا ہوں؟', 'It shades the sun.', 'سورج کو ڈھانپ لیتا ہے۔', array['cloud','بادل']),
  (21, 'You can catch me, but never throw me. What am I?', 'مجھے پکڑ تو سکتے ہو، پھینک نہیں سکتے۔ کیا؟', 'It comes with sneezes.', 'چھینکوں کے ساتھ آتا ہے۔', array['cold','a cold','زکام','نزلہ']),
  (22, 'Where does today come before yesterday?', 'کہاں «آج» «کل» سے پہلے آتا ہے؟', 'A book of words.', 'الفاظ کی کتاب۔', array['dictionary','لغت','ڈکشنری']),
  (23, 'I pass through cities and fields but never move. What am I?', 'شہروں اور کھیتوں سے گزروں، مگر ہلوں نہیں۔ کیا؟', 'Travellers use it daily.', 'مسافروں کی ساتھی۔', array['road','سڑک','راستہ']),
  (24, 'I wear a yellow coat and bow politely before you enjoy me. What am I?', 'پیلا چولا پہنوں، کھانے سے پہلے جھک جاؤں۔ کیا؟', 'Monkeys agree.', 'بندروں کی پسند۔', array['banana','کیلا']),
  (25, 'I dance in the sky on a single string. What am I?', 'ایک ڈور پر آسمان میں ناچوں۔ میں کیا ہوں؟', 'Basant knows me well.', 'بسنت کی رونق۔', array['kite','پتنگ']),
  (26, 'The more I work, the smaller I become. What am I?', 'جتنا کام کروں، اتنا گھٹتا جاؤں۔ میں کیا ہوں؟', 'It bubbles as it works.', 'جھاگ بنا کر کام کرتا ہے۔', array['soap','صابن']),
  (27, 'Round as a plate, I light the night sky. What am I?', 'تھالی جیسا گول، رات کے آسمان کا چراغ۔ کیا؟', 'Eid begins when it is seen.', 'عید اس کے دیکھنے سے۔', array['moon','چاند']),
  (28, 'I have a head and a tail but no body. What am I?', 'سر ہے، دُم ہے، مگر جسم نہیں۔ میں کیا ہوں؟', 'It jingles in a pocket.', 'جیب میں کھنکتا ہے۔', array['coin','سکہ']),
  (29, 'The hotter the day, the higher I climb. What am I?', 'دن جتنا گرم، میں اتنا اوپر چڑھوں۔ کیا ہوں؟', 'It measures the summer.', 'گرمی ناپنے والا۔', array['thermometer','mercury','تھرمامیٹر','پارہ'])
)
...
-- ----------------------------------------------------------------------------
-- Community post types for games: an open table to claim, and a
-- solved-riddle share.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- pg_cron sweep, once a minute — sessions keep moving even with
-- nobody watching (away seats, bot tables). Clients also call
-- game_tick(session) directly so the on-screen countdown resolves
-- instantly. If pg_cron cannot be enabled, that client path alone
-- still upholds every rule.
-- ----------------------------------------------------------------------------
do $$
begin
