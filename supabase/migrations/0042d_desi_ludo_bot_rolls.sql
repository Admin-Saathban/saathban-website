-- ============================================================================
-- 0042d — a bot seat rolls for itself
--
-- THE BUG THIS FIXES. Ludo's roll is a separate RPC from its move, because a
-- person rolls, looks, and then decides. `ludo_roll` checks auth.uid() against
-- the seat, which is right for a person and impossible for a bot: a bot seat
-- has no profile_id, and nobody is signed in as it.
--
-- game_tick plays a bot seat (or a seat whose person has gone quiet) by
-- calling exec_game_move(session, seat, true, null) — straight to the move,
-- with no roll before it. The executor found no dice, raised 'Roll first',
-- and game_tick's `exception when others` swallowed it as a skipped session.
-- The visible result would have been a table frozen for good on a bot's turn,
-- with nothing in the UI to say why. Every ludo table with a bot in it, and
-- every table where somebody put their phone down, would eventually stop.
--
-- So: when the caller IS a bot and there are no dice, the executor rolls its
-- own. Not by calling ludo_roll — that would fail the same auth check — but
-- with the same arithmetic inline, chain and all.
--
-- ALSO CORRECTED HERE, in the comment rather than the code: 0042's header
-- claimed "a turn that lapses mid-chain voids the chain". That was my
-- decision, and the rails had already made a different and better one. A
-- lapsed turn is not forfeited in this game — game_tick hands the seat to the
-- bot, which keeps playing it. So a chain interrupted by someone's attention
-- wandering is continued by the bot, and it ends the way every chain ends: at
-- 3, 6 or 9, or when the sixes stop coming. There is no separate timeout rule,
-- because there are no lapsed turns to have one about.
-- ============================================================================
...
begin
  ...
  if v_dice is null and p_by_bot then
    ...
    if not v_any then
      -- Nothing to do. The rails advance the turn on again = false; this
      -- writes one honest "rolled, could not move" row and stops.
      v_state := public.ludo_resolve_chain(v_state);
  ...
  -- best piece for it by the existing heuristic where that applies.
    ...
    for v_i in 0 .. jsonb_array_length(v_dice) - 1 loop
      if not coalesce((v_dice -> v_i ->> 'used')::boolean, false)
        ...
        if jsonb_array_length(v_legal) > 0 then
          -- Prefer a capture, then the piece furthest along: the same
          -- ordering ludo_bot_pick uses, applied to the first option
          -- that captures if one does.
